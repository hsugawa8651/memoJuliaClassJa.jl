...
* テキスト内の立体 (roman style) `$a\;\mathrm{over}\;b$` → $a\;\mathrm{over}\;b$
...
独立変数 (independent variable) $x$ の取りうる値からなる集合である。ちなみに、従属変数 (dependent variable) $y$ が取りうる値からなる集合を、値域 (range)という
...
上のプログラムを、for文を用いた繰り返しとして書き直してみよ。
...
* [Base.log — Method](https://docs.julialang.org/en/v1.1/base/math/#Base.log-Tuple{Number,Number})
