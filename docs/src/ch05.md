...
最初の if文の論理式が `false` なら、
elseif文の論理式を計算し、それが `true` なら、elseif 文の次の文から、`elseif`または`end` が出現するまでの文 (elseifブロック)を実行する。
...
上のfor文の繰り返しは、３重の入れ子になっている。これは、一つのfor文にまとめられる。
...
関数`plot`のfmtパラメータを `.` から `o` (circle)に変えてみよ。
...
図形の中に入った数を数えよう。
両者の割合から、図形の面積を推定できる。
これを、モンテカルロ (Monte Carlo)法による面積の算出法という。
